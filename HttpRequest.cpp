...
void HttpRequest::setCallback(HTTP_REQ_CALLBACK) {
...
void HttpRequest::parseRequest(char c){
  ...
  switch(parseStatus){
    ...
    case HTTP_NEW_LINE:
      ...
      else { if(strcmp(method,"POST")==0 && dataBlockLength>0) parseStatus=HTTP_POST_NAME; //empty line
...
void HttpRequest::addParam(){
  ...
  if(callback) {
    callback(HTTP_REQ_EVENT_PARAM, tmpParamName, tmpParamValue);
  }
